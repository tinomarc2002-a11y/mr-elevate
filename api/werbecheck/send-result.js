...
function buildBudgetPlan({ metaPct, googlePct, budgetBucket }) {
  ...
  if (strategy.mode === 'mix') {
    return `<p style="margin:0;">Zur Einordnung ein Rechenbeispiel für euren Rahmen (${info.label}): Bei <strong>${euro(info.example)} im Monat</strong> würden wir etwa <strong>${euro(info.example * googlePct / 100)} auf Google Ads</strong> und <strong>${euro(info.example * metaPct / 100)} auf Meta Ads</strong> geben – zusammen rund ${euro(daily)} Tagesbudget.</p>`;
  }

  return `<p style="margin:0;">Zur Einordnung ein Rechenbeispiel für euren Rahmen (${info.label}): Bei <strong>${euro(info.example)} im Monat</strong> würden wir zunächst das <strong>gesamte Budget auf ${strategy.dominant} Ads</strong> konzentrieren, also rund ${euro(daily)} Tagesbudget. Ein Kanal mit ausreichend Budget schlägt zwei Kanäle, die beide zu dünn laufen – die Plattformen brauchen eine Mindestmenge an Daten, um überhaupt sinnvoll auszusteuern.</p>`;
}
...
function buildEmailFooter(consentRecord) {
  const consentNote = consentRecord ? `
      Du hast am ${escapeHtml(new Date(consentRecord.timestamp).toLocaleString('de-DE', { timeZone: 'Europe/Berlin' }))} Uhr eingewilligt, dass wir dich
...
function buildLeadEmailHtml({ name, qualified, reasons, metaPct, googlePct, budgetBucket, topInsights, extraChannels, consentRecord }) {
  ...
  if (!qualified) {
    ...
    return `
      ${MAIL_WRAP_OPEN}
        ...
        <p style="margin:0 0 14px;">danke, dass du dir die Zeit für den Werbe-Check genommen hast. Wir könnten dir an dieser Stelle eine Kampagne verkaufen – in eurer aktuellen Situation wäre das aber nicht ehrlich. Deshalb bekommst du hier die Einschätzung, die wir auch einem Bekannten geben würden.</p>
        <p style="margin:0 0 8px;"><strong>Von bezahlter Werbung würden wir euch derzeit abraten:</strong></p>
        <ul style="padding-left:20px;color:#3B4A60;margin:0;">${reasonItems}</ul>

        ${h3('Was wir euch stattdessen empfehlen')}
        ...
        <p style="margin:0;">Sobald sich einer dieser beiden Punkte bei euch verändert, lohnt sich ein neuer Blick. Melde dich dann einfach – wir schauen kostenlos drauf.</p>
  ...
  return `
    ${MAIL_WRAP_OPEN}
      <h2 style="color:#14203A;font-size:22px;margin:0 0 16px;">Hallo ${anrede},</h2>
      <p style="margin:0 0 14px;">danke für deine Teilnahme am Werbe-Check. Wir haben deine zwölf Antworten ausgewertet – hier ist das Ergebnis, inklusive Begründung, konkretem Budget-Vorschlag und den ersten Schritten.</p>
      ...
      <p style="margin:0 0 18px;">Genau dafür gibt es die <strong>kostenlose Potenzialanalyse</strong>: 30 Minuten, in denen wir eure Antworten konkret anschauen und eine einzige Frage klären – lässt sich bei euch mit Werbung sinnvoll etwas bewegen, und womit fängt man an? <strong>Kein Verkaufsgespräch:</strong> Wenn ich sehe, dass es gerade nicht passt, sage ich dir das im Termin.</p>
...
module.exports = async function handler(req, res) {
  ...
  if (!body.consent || body.consent.given !== true) {
    res.status(400).json({ error: 'Ohne Einwilligung zur Kontaktaufnahme können wir die Anfrage nicht verarbeiten.' });
    return;
  }
